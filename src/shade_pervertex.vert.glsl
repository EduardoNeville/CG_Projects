...
void main() {
	...
	*/
	...
	vec4 vertex_normal_view = vec4(mat_normals_to_view*vertex_normal,1);
	...
	vec4 half_vec = (eye_position + n_light_dir) / length(eye_position + n_light_dir);
	
	spec_dot = dot(half_vec, n_obj_norm);
	
	vec3 mat_s_dot = material_color * pow(spec_dot, material_shininess);

	if (dot(reflected_light, eye_position) < 0.){
		mat_s_dot = vec3(0.);
	}
